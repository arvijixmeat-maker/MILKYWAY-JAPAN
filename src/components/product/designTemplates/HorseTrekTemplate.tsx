...
function Slot({ src, label, editing, alt }: { src: string; label: string; editing?: boolean; alt: string }) {
    ...
    if (editing) {
        return (
            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px dashed rgba(2,159,133,0.55)', borderRadius: 'inherit', color: TEAL, fontSize: 17, fontWeight: 700, textAlign: 'center', padding: 12, boxSizing: 'border-box', background: 'rgba(6,196,160,0.06)' }}>
                {label}
            </div>
        );
    }
        ...
        <div className="ht-design" style={{ width: 860, margin: '0 auto', background: '#fff', color: '#2b2b2b', overflow: 'hidden', fontFamily: '"Noto Sans JP", "Hiragino Kaku Gothic ProN", -apple-system, sans-serif', WebkitFontSmoothing: 'antialiased' }}>
            ...
            <section style={{ position: 'relative', height: 1080, overflow: 'hidden', background: DEEP }}>
                {opBgs.length > 0 ? opBgs.map((src, i) => (
                    ...
                    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.5)', fontSize: 20, fontWeight: 700 }}>
                ...
                <div style={{ position: 'relative', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '150px 0 70px', boxSizing: 'border-box' }}>
                    <div style={{ fontSize: 34, fontWeight: 500, letterSpacing: '0.06em', color: 'rgba(255,255,255,0.62)' }}>{v('op_line1')}</div>
                    ...
                    <div style={{ fontSize: 74, fontWeight: 800, letterSpacing: '-0.02em', color: '#fff', textShadow: '0 4px 30px rgba(0,0,0,0.5)' }}>{v('op_line2')}</div>
            ...
            <section style={{ position: 'relative', background: '#02100E', overflow: 'hidden' }}>
                <div style={{ position: 'relative', height: 960, overflow: 'hidden' }}>
                    ...
                    <div style={{ position: 'relative', padding: '96px 60px 0', textAlign: 'center' }}>
                        <div style={{ fontSize: 30, fontWeight: 700, letterSpacing: '-0.02em', color: 'rgba(255,255,255,0.86)' }}>{v('hero_kicker')}</div>
                        <h2 style={{ margin: '40px 0 0', fontSize: 82, lineHeight: 1.14, fontWeight: 800, letterSpacing: '-0.04em', color: '#fff' }}>{v('hero_title')}</h2>
                        <Lines text={v('hero_body')} style={{ margin: '44px 0 0', fontSize: 26, lineHeight: 1.75, fontWeight: 600, letterSpacing: '-0.015em', color: 'rgba(255,255,255,0.8)', textShadow: '0 2px 18px rgba(0,0,0,0.5)' }} />
                    </div>
                </div>
            ...
            <section style={{ position: 'relative', height: 1000, overflow: 'hidden', background: DEEP }}>
                ...
                <div style={{ position: 'relative', padding: '120px 70px', color: '#fff' }}>
                    <h2 style={{ margin: 0, fontSize: 62, lineHeight: 1.05, fontWeight: 800, letterSpacing: '-0.045em', textShadow: '0 3px 20px rgba(0,0,0,0.3)' }}>{v('intro_title')}</h2>
                    <Lines text={v('intro_body')} style={{ margin: '38px 0 0', fontSize: 29, lineHeight: 1.62, fontWeight: 600, letterSpacing: '-0.02em', textShadow: '0 2px 14px rgba(0,0,0,0.35)', textWrap: 'pretty' } as React.CSSProperties} />
                </div>
            </section>

            {/* ── 04 이용자 특전 ────────────────────────────── */}
            <section style={{ background: '#fff', padding: '100px 40px 110px' }}>
                <h2 style={{ margin: 0, textAlign: 'center', fontSize: 51, fontWeight: 800, letterSpacing: '-0.05em', color: DEEP }}>{v('perks_title')}</h2>
                <div style={{ margin: '22px 0 0', textAlign: 'center', fontSize: 24, fontWeight: 600, color: '#5a5a5a', letterSpacing: '-0.02em' }}>{v('perks_sub')}</div>
                    ...
                    {[1, 2, 3, 4, 5, 6].map(n => (
                        <div key={n} style={{ position: 'relative', height: 250, borderRadius: 20, overflow: 'hidden', background: '#EFFEF9' }}>
                            ...
                            <div style={{ position: 'absolute', inset: 'auto 0 0 0', pointerEvents: 'none', padding: '26px 16px 22px', background: 'linear-gradient(180deg, rgba(0,51,46,0) 0%, rgba(0,51,46,0.72) 100%)', textAlign: 'center', color: '#fff', wordBreak: 'keep-all' }}>
                                <div style={{ fontSize: 17, fontWeight: 800, letterSpacing: '-0.03em' }}>{v(`perk${n}_title`)}</div>
                                <div style={{ marginTop: 6, fontSize: 19, fontWeight: 600, letterSpacing: '-0.02em' }}>{v(`perk${n}_sub`)}</div>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <div style={{ marginTop: 20, display: 'grid', gridTemplateColumns: '1fr 1fr', alignItems: 'stretch', border: '1px solid #C8FFEF', borderRadius: 20, overflow: 'hidden' }}>
                    ...
                    <div style={{ padding: '44px 34px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 26, textAlign: 'center', wordBreak: 'keep-all', textWrap: 'pretty' } as React.CSSProperties}>
                        <div style={{ fontSize: 24, fontWeight: 800, color: DEEP, letterSpacing: '-0.02em' }}>{v('perk_air_title')}</div>
                        <Lines text={v('perk_air_note')} style={{ fontSize: 21, lineHeight: 1.6, fontWeight: 600, color: '#5a5a5a', letterSpacing: '-0.02em' }} />
                    </div>
                </div>
            </section>
            ...
            <section style={{ background: '#fff', padding: '110px 50px 120px', wordBreak: 'keep-all' }}>
                <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: 40, fontWeight: 700, letterSpacing: '-0.04em', color: '#9a9a9a' }}>{v('worry_kicker')}</div>
                    <h2 style={{ margin: '14px 0 0', fontSize: 43, fontWeight: 800, letterSpacing: '-0.05em', color: '#2b2b2b' }}>{v('worry_title')}</h2>
                </div>
                <div style={{ marginTop: 70, display: 'flex', flexDirection: 'column', gap: 34 }}>
                    {([
                        ...
                        <div key={b.key} style={{ display: 'flex', justifyContent: b.align }}>
                            <div className={`ht-bubble ${b.tail}`} style={{ maxWidth: b.max, padding: '24px 34px', borderRadius: 22, background: '#F1F1F1', fontSize: b.size, lineHeight: 1.45, fontWeight: 600, letterSpacing: '-0.03em', color: '#3a3a3a', whiteSpace: 'pre-line' }}>
                                {v(b.key)}
                            </div>
                        </div>
                    ))}
            ...
            <section style={{ background: '#fff', padding: '100px 40px 96px', wordBreak: 'keep-all' }}>
                <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: 23, fontWeight: 700, color: MINT, letterSpacing: '-0.02em' }}>{v('ger_kicker')}</div>
                    <div style={{ marginTop: 14, fontSize: 30, fontWeight: 600, color: '#5a5a5a', letterSpacing: '-0.03em' }}>{v('ger_sub')}</div>
                    <h2 style={{ margin: '20px 0 0', fontSize: 52, lineHeight: 1.2, fontWeight: 800, letterSpacing: '-0.045em', color: DEEP }}>
                        <div style={{ fontSize: 49 }}>{v('ger_title1')}</div>
                ...
                <div style={{ marginTop: 38, display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '16px 40px' }}>
                    {[v('ger_check1'), v('ger_check2'), v('ger_check3')].filter(Boolean).map((c, i) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 24, fontWeight: 800, letterSpacing: '-0.03em', color: DEEP }}>
                            <div style={{ color: MINT }}>✓</div>
                            <div>{c}</div>
                        </div>
                    ))}
                </div>
                <div style={{ marginTop: 24, textAlign: 'center', fontSize: 20, fontWeight: 600, color: '#5a5a5a', letterSpacing: '-0.02em' }}>{v('ger_note')}</div>
            </section>
            ...
            <section style={{ position: 'relative', background: 'linear-gradient(180deg, #C8FFEF 0%, #EFFEF9 32%, #EFFEF9 100%)', paddingBottom: 90 }}>
                <div style={{ padding: '110px 70px 60px', textAlign: 'center' }}>
                    ...
                    <h2 style={{ margin: '18px 0 0', fontSize: 52, lineHeight: 1.32, fontWeight: 800, letterSpacing: '-0.04em', color: '#2b2b2b' }}>{v('route_intro_title')}</h2>
                    <Lines text={v('route_intro_p1')} style={{ margin: '46px 0 0', fontSize: 26, lineHeight: 1.75, fontWeight: 500, color: '#4a4a4a', letterSpacing: '-0.02em' }} />
                    <Lines text={v('route_intro_p2')} style={{ margin: '38px 0 0', fontSize: 26, lineHeight: 1.75, fontWeight: 500, color: '#4a4a4a', letterSpacing: '-0.02em' }} />
                </div>
                ...
                <div style={{ margin: '-60px 40px 0', position: 'relative', background: '#fff', borderRadius: 40, boxShadow: '0 24px 60px rgba(0,51,46,0.10)', padding: '70px 56px 76px' }}>
                    <h3 style={{ margin: 0, textAlign: 'center', fontSize: 61, lineHeight: 1.4, fontWeight: 800, letterSpacing: '-0.035em', color: '#2b2b2b' }}>{v('route_card_title')}</h3>

                    <div style={{ margin: '56px 0 64px', display: 'flex', justifyContent: 'center' }}>
                        <iframe src={mapUrl} title="몽골 지도 위 여정 지점" scrolling="no" style={{ width: '100%', maxWidth: 760, aspectRatio: '760/990', border: 0, display: 'block' }} loading="lazy" />
                    </div>

                    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 20 }}>
                        <div style={{ fontSize: 26, fontWeight: 800, letterSpacing: '-0.035em', color: DEEP }}>{v('spots_label')}</div>
                        <div style={{ fontSize: 15, fontWeight: 600, color: TEAL, letterSpacing: '-0.02em' }}>{v('spots_hint')}</div>
                    </div>
                        ...
                        {dayCards.map((c, i) => (
                            <div key={i} style={{ flex: '0 0 300px', marginTop: c.offset ? 34 : 0, scrollSnapAlign: 'start', background: '#fff', borderRadius: 26, boxShadow: '0 14px 34px rgba(0,51,46,0.10)', overflow: 'hidden' }}>
                                <div style={{ position: 'relative', height: 190, background: '#C8FFEF' }}>
                                    ...
                                    <div style={{ position: 'absolute', top: 14, left: 14, pointerEvents: 'none', padding: '7px 16px', borderRadius: 999, background: MINT, color: '#fff', fontSize: 17, fontWeight: 800, letterSpacing: '-0.03em' }}>{c.badge}</div>
                                </div>
                                <div style={{ padding: '22px 24px 28px' }}>
                                    <div style={{ fontSize: 22, fontWeight: 800, letterSpacing: '-0.03em', color: DEEP }}>{c.title}</div>
                                    <Lines text={c.body} style={{ margin: '12px 0 0', fontSize: 19, lineHeight: 1.6, fontWeight: 500, color: '#5a5a5a', letterSpacing: '-0.02em' }} />
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <section style={{ position: 'relative', height: 1180, overflow: 'hidden', background: '#0A6558' }}>
                ...
                <div style={{ position: 'relative', pointerEvents: 'none', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '120px 60px 80px', boxSizing: 'border-box', textAlign: 'center' }}>
                    <div style={{ fontSize: 27, fontWeight: 600, color: 'rgba(255,255,255,0.9)', letterSpacing: '-0.02em', textShadow: '0 2px 14px rgba(0,0,0,0.4)' }}>{v('gobi_kicker')}</div>
                    <h2 style={{ margin: '26px 0 0', fontSize: 62, lineHeight: 1.18, fontWeight: 800, letterSpacing: '-0.045em', color: '#fff', textShadow: '0 3px 22px rgba(0,0,0,0.45)' }}>{v('gobi_title')}</h2>
                    <div style={{ marginTop: 14, width: 520, height: 5, borderRadius: 3, background: MINT }} />
                    <Lines text={v('gobi_body')} style={{ margin: '34px 0 0', fontSize: 38, lineHeight: 1.42, fontWeight: 700, letterSpacing: '-0.035em', color: '#fff', textShadow: '0 2px 18px rgba(0,0,0,0.45)' }} />
                </div>
            </section>
            ...
            <section style={{ background: '#fff', padding: '96px 60px 100px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 22 }}>
                    ...
                    <div style={{ padding: '16px 44px', border: '1px solid #91FEE0', borderRadius: 999, fontSize: 37, fontWeight: 800, letterSpacing: '-0.035em', color: '#2b2b2b' }}>{v('hl_title')}</div>
                    <div style={{ flex: '0 0 10px', height: 10, borderRadius: '50%', background: MINT }} />
                    <div style={{ flex: 1, height: 1, background: '#e6e6e6' }} />
                </div>
                <div style={{ marginTop: 22, textAlign: 'right', fontSize: 15, fontWeight: 600, color: TEAL, letterSpacing: '-0.02em' }}>{v('hl_hint')}</div>
                <div className="ht-scroll" style={{ margin: '22px -60px 0 0', display: 'flex', alignItems: 'flex-start', gap: 24, overflowX: 'auto', scrollSnapType: 'x mandatory', scrollbarWidth: 'none', msOverflowStyle: 'none', padding: '4px 60px 12px 0' } as React.CSSProperties}>
                    {points.map((p, i) => (
                        <div key={i} style={{ flex: '0 0 340px', scrollSnapAlign: 'start', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start' }}>
                            ...
                            <div style={{ marginTop: 20, textAlign: 'center', fontSize: 24, lineHeight: 1.45, letterSpacing: '-0.025em', color: '#3a3a3a' }}>{p.caption}</div>
                        </div>
                    ))}
                </div>
                <div style={{ marginTop: 88, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20 }}>
                    ...
                    <div style={{ fontSize: 99, fontWeight: 800, letterSpacing: '-0.04em', color: DEEP }}>{v('and_word')}</div>
            ...
            <section style={{ position: 'relative', height: 720, overflow: 'hidden', background: DEEP }}>
                ...
                <div style={{ position: 'absolute', left: 0, right: 0, bottom: 130, pointerEvents: 'none', textAlign: 'center', color: '#fff', fontSize: 56, lineHeight: 1.6, fontWeight: 600, letterSpacing: '-0.02em', textShadow: '0 2px 16px rgba(0,0,0,0.6)' }}>{v('mw_caption')}</div>
            </section>

            {/* ── 12 공항 도착 ──────────────────────────────── */}
            <section style={{ background: '#fff', padding: '100px 0 0', wordBreak: 'keep-all' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    <div style={{ fontSize: 38, fontWeight: 700, letterSpacing: '-0.04em', color: '#c9c9c9' }}>{v('arr_from')}</div>
                    ...
                    <div style={{ marginTop: 34, fontSize: 60, fontWeight: 800, letterSpacing: '-0.05em', color: DEEP }}>{v('arr_to')}</div>
                </div>
                ...
                <div style={{ padding: '0 60px' }}>
                    <Lines text={v('arr_lead')} style={{ textAlign: 'center', fontSize: 35, fontWeight: 700, letterSpacing: '-0.03em', color: '#2b2b2b' }} />
                    <div style={{ marginTop: 48, display: 'flex', flexDirection: 'column', gap: 24, alignItems: 'center' }}>
                        <Lines text={v('arr_body')} style={{ fontSize: 20, fontWeight: 800, letterSpacing: '-0.035em', color: '#2b2b2b', textAlign: 'center' }} />
                    </div>
                </div>

                <div style={{ marginTop: 56, padding: '44px 60px', background: '#C8FFEF', textAlign: 'center' }}>
                    <div style={{ fontSize: 25, fontWeight: 800, letterSpacing: '-0.035em', color: DEEP }}>{v('arr_band1')}</div>
                    <div style={{ marginTop: 12, fontSize: 21, fontWeight: 700, letterSpacing: '-0.035em', color: '#0A6558' }}>{v('arr_band2')}</div>
                </div>
                ...
                <div style={{ padding: '70px 60px 100px' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'center', gap: 20 }}>
                        ...
                        <Lines text={v('welcome_side')} style={{ flex: '0 0 auto', paddingTop: 130, whiteSpace: 'pre-line', fontSize: 24, fontWeight: 700, letterSpacing: '-0.03em', color: '#2b2b2b' }} />
                    </div>
                    <div style={{ marginTop: 34, textAlign: 'center', fontSize: 27, lineHeight: 1.5, fontWeight: 700, letterSpacing: '-0.035em', color: '#2b2b2b' }}>{v('welcome_caption')}</div>
                </div>
            </section>
            ...
            <section style={{ position: 'relative', background: '#EFFEF9', padding: '60px 0 90px' }}>
                <div style={{ position: 'relative', height: 470, overflow: 'hidden' }}>
                    ...
                    <div style={{ position: 'relative', pointerEvents: 'none', padding: '56px 60px 0' }}>
                        <h2 style={{ margin: 0, fontSize: 46, fontWeight: 800, letterSpacing: '-0.04em', color: '#fff', textShadow: '0 2px 18px rgba(0,0,0,0.35)' }}>{v('d1_title')}</h2>
                            ...
                            {tabs.map((t, i) => (
                                <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 8, opacity: i === 0 ? 1 : 0.72 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                        ...
                                        <div style={{ fontSize: 21, fontWeight: 800, color: '#fff', letterSpacing: '-0.02em' }}>{t.label}</div>
                                    </div>
                                    <div style={{ fontSize: 19, fontWeight: i === 0 ? 700 : 600, color: '#fff', letterSpacing: '-0.02em' }}>{t.text}</div>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div style={{ margin: '-70px 40px 0', position: 'relative', background: '#fff', borderRadius: 40, boxShadow: '0 24px 60px rgba(0,51,46,0.10)', padding: '60px 56px 66px', wordBreak: 'keep-all' }}>
                    <div style={{ fontSize: 26, fontWeight: 700, color: DEEP, letterSpacing: '-0.03em' }}>{v('d1_kicker')}</div>
                    <h3 style={{ margin: '16px 0 0', fontSize: 46, fontWeight: 800, letterSpacing: '-0.045em', color: TEAL }}>{v('d1_head')}</h3>
                    <Lines text={v('d1_body')} style={{ margin: '34px 0 0', fontSize: 23, lineHeight: 1.75, fontWeight: 500, color: '#4a4a4a', letterSpacing: '-0.02em', textWrap: 'pretty' } as React.CSSProperties} />
                    ...
                    <div style={{ marginTop: 52, display: 'grid', gridTemplateColumns: '1.15fr 1fr', gap: 44 }}>
                        <div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                <SectionDot />
                                <div style={{ fontSize: 26, fontWeight: 800, letterSpacing: '-0.03em', color: DEEP }}>{v('d1_route_title')}</div>
                            </div>
                            <div style={{ marginTop: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
                                {schedule.map((s, i) => (
                                    <div key={i} style={{ display: 'flex', gap: 20 }}>
                                        <div style={{ flex: '0 0 82px', fontSize: 21, fontWeight: 800, color: TEAL, letterSpacing: '-0.02em' }}>{s.time}</div>
                                        <Lines text={s.body} style={{ fontSize: 21, lineHeight: 1.55, fontWeight: 600, color: '#3a3a3a', letterSpacing: '-0.02em' }} />
                                    </div>
                                ))}
                            </div>
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 36 }}>
                            ...
                            <div>
                                ...
                                <div style={{ marginTop: 20, fontSize: 21, fontWeight: 600, color: '#3a3a3a', letterSpacing: '-0.02em' }}>{v('d1_stay')}</div>
                                <div style={{ marginTop: 14, display: 'flex', flexWrap: 'wrap', gap: '10px 22px', fontSize: 19, fontWeight: 600, color: TEAL, letterSpacing: '-0.02em' }}>
                    ...
                    <div style={{ marginTop: 36 }}>
                        ...
                        <div style={{ marginTop: 18, fontSize: 21, fontWeight: 600, color: '#3a3a3a', letterSpacing: '-0.02em' }}>{v('d1_exp')}</div>
                    </div>
                </div>
            </section>
        </div>
