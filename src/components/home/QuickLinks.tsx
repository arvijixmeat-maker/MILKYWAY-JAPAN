...
import businessIcon from '../../assets/business_estimate.png';
...
export const QuickLinks: React.FC = () => {
    ...
    const DEFAULT_LINKS: QuickLink[] = useMemo(() => [
        ...
        { id: 'link-4', icon: 'business_center', image: businessIcon, label: t('home.quick_links.business'), path: '/business-estimate' },
    ...
    const { data: links = DEFAULT_LINKS } = useQuery({
        queryKey: ['quickLinks', t('home.quick_links.products')], // Add dependency
        queryFn: async () => {
            const { data } = await supabase.from('quick_links').select('*').order('order');
