...
export const GobiSection: React.FC = () => {
        ...
        const fetchProducts = async () => {
            try {
                ...
                if (Array.isArray(data)) {
                    const gobiData = data.filter((p: any) => p.category === 'Í≥†ÎπÑ?¨Îßâ').slice(0, 4);
    ...
    const gobiProducts = products.filter(p => p.category === 'Í≥†ÎπÑ?¨Îßâ').slice(0, 4);
    ...
    return (
        <section className="py-10 bg-slate-900 relative overflow-hidden">
            ...
            <div className="relative z-10 px-5 mb-6 flex items-end justify-between">
                <div>
                    <h3 className="text-xl font-extrabold text-white">Á•ûÁßò??Ç¥?ìÁ†ÇÊº?/h3>
                    <p className="text-[13px] text-white/60 mt-1 text-shadow-sm">?Ç‰∏ò?®Èôç?äÊ≥®?êÊòüÁ©∫„ÅÆÈ•óÂ?</p>
                </div>
                <button onClick={() => navigate('/products?category=gobi')} className="text-primary text-sm font-bold flex items-center">?ô„Åπ??¶ã??<span className="material-symbols-outlined text-sm">chevron_right</span></button>
            </div>
            ...
            <div className="relative z-10 flex overflow-x-auto gap-4 px-5 pb-4 scrollbar-hide snap-x snap-mandatory">
                {gobiProducts.length > 0 ? (
                            ...
                            <div className="relative aspect-[4/5] rounded-3xl overflow-hidden border border-white/10 group">
                                ...
                                <div className="absolute bottom-5 left-5 right-5">
                                    ...
                                    <span className="inline-block px-2 py-0.5 bg-white/20 backdrop-blur-md rounded text-[10px] text-white font-bold mb-2">
                                        {product.isPopular ? '‰∫∫Ê∞ó?≥„Éº?? : (product.tags?.[0] || '?¥„Éì?ÇÊº†')}
                                    </span>
                    ...
                    <div className="text-white/60 text-sm py-10 px-5">?ªÈå≤?ï„Çå?üÂïÜ?Å„Åå?Ç„Çä?æ„Åõ?ì„Ä?/div>
                )}
            </div>
        </section>
    );
