            ...
            <div className="px-5 mb-5 flex flex-col gap-1.5">
                <div className="flex items-start justify-between gap-4">
                    <h3 className="text-[17px] sm:text-[19px] font-black text-[#0e1a18] dark:text-white leading-snug break-keep flex-1">
                        {t('home.magazine.title', { defaultValue: '‰ªäÂá∫?∫„Åô?ãËâØ?ÑÊóÖË°å„Ç≥?º„Çπ?ä„Åô?ô„ÇÅÔº? })}
                    </h3>
                    <button
                    ...
                    >
                        {t('home.magazine.view_all', { defaultValue: '?®„Å¶Ë¶ã„Çã' })} <span className="material-symbols-outlined text-base sm:text-lg ml-0.5">chevron_right</span>
                    </button>
                </div>
                <p className="text-[12px] sm:text-[13px] text-gray-500">
                    {t('home.magazine.subtitle', { defaultValue: '?¢„É≥?¥„É™?¢„ÅÆÂ§©„ÅÆÂ∑ù„Åå?≥ÈÅ∏?ó„Åü?ÄÈ´ò„ÅÆ??öÑ?? })}
                </p>
            </div>
                    ...
                    >
                        ...
                        <div className="absolute bottom-0 left-0 w-full p-6 text-left z-10">
                            ...
                            <span className="inline-block px-2.5 py-1 bg-white/30 backdrop-blur-sm rounded-md text-[11px] font-bold text-white mb-3">
                                {item.category || t('home.magazine.default_category', { defaultValue: 'Ê∫ñÂÇô' })}
                            </span>
