        ...
        const fetchProducts = async () => {
            try {
                ...
                if (Array.isArray(data)) {
                    const centralData = data.filter((item: any) => item.category === 'Ï§ëÏïôÎ™ΩÍ≥®' && item.status === 'active').slice(0, 4);
    ...
    return (
        <section className="py-8 px-5 border-t border-slate-100 dark:border-slate-800">
            <div className="flex items-end justify-between mb-5">
                <div>
                    <h3 className="text-xl font-extrabold text-slate-900 dark:text-white">‰∏?§Æ?¢„É≥?¥„É´ ??É©?∑„ÉÉ??/h3>
                    <p className="text-[13px] text-slate-500 mt-1">?í„ÅÑ?âÂéü?®Ê??≤„Åå?±Â≠ò?ô„Çã?¥Ê?</p>
                </div>
                <button
                    onClick={() => navigate('/products?category=central')}
                    className="text-primary text-sm font-bold flex items-center"
                >
                    ?Ç„Å£?®Ë¶ã??<span className="material-symbols-outlined text-sm">chevron_right</span>
                </button>
            </div>
