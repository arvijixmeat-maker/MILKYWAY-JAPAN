...
export const HovsgolSection: React.FC = () => {
        ...
        const fetchProducts = async () => {
            try {
                ...
                if (Array.isArray(data)) {
                    const khuvsgulData = data.filter((p: any) => p.category === '?âÏä§Íµ?).slice(0, 3);
    ...
    const khuvsgulProducts = products.filter(p => p.category === '?âÏä§Íµ?).slice(0, 3);

    return (
        <section className="py-8 px-5 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-background-dark">
            <div className="flex items-end justify-between mb-6">
                <div>
                    <h3 className="text-xl font-extrabold text-slate-900 dark:text-white">?í„ÅÑ?üÁè† ?ï„Éñ?π„Ç∞??/h3>
                    <p className="text-[13px] text-slate-500 mt-1">?¢„É≥?¥„É´??Øç?®Âëº?∞„Çå?ãÂ∑®Â§ß„Å™Êπ?/p>
                </div>
                <button onClick={() => navigate('/products?category=khuvsgul')} className="text-primary text-sm font-bold flex items-center">?ô„Åπ??¶ã??<span className="material-symbols-outlined text-sm">chevron_right</span></button>
            </div>
            <div className="space-y-4">
                {khuvsgulProducts.length > 0 ? (
                            ...
                            <div onClick={() => navigate(`/products/${product.id}`)} className="flex gap-4 group cursor-pointer">
                                ...
                                <div className="flex flex-col justify-center py-1 flex-1">
                                    <span className="text-[10px] font-bold text-primary mb-1">
                                        {product.isPopular ? '‰∫∫Ê∞ó?•‰∏ä?? : (product.tags?.[0] || '?ä„Åô?ô„ÇÅ')}
                                    </span>
                                    ...
                                    <p className="text-[12px] text-slate-500 mb-2 line-clamp-1">
                                        {product.duration} ¬∑ {product.highlights?.[0]?.title || '?êÎßÅ ?¨Ìñâ'}
                                    </p>
                    ...
                    <div className="text-center py-8 text-slate-400 text-sm">
                        ?ªÈå≤?ï„Çå?ü„Éï?ñ„Çπ?∞„É´?ÜÂìÅ?å„ÅÇ?ä„Åæ?õ„Çì??
                    </div>
                )}
            </div>
        </section>
