...
/**
 * Admin page to edit the site-wide "Tour Common FAQ" displayed at the
 * bottom of every product detail page. One global list — saving here
 * updates the FAQ block for ALL tours (unless that tour has its own
 * product-specific FAQs, which still take precedence).
 */
export const AdminTourFAQManage: React.FC = () => {
    ...
    return (
        <div className={`flex min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 font-sans ${isDarkMode ? 'dark' : ''}`}>
            <AdminSidebar activePage="tour-faqs" isDarkMode={isDarkMode} toggleTheme={toggleTheme} />
            ...
            <main className="ml-64 flex-1 flex flex-col min-h-screen">
                <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-40 px-8 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                            여기서 한 번 작성하면 모든 상품 상세 페이지 하단의 FAQ 섹션에 자동으로 표시됩니다.
                        </p>
                    </div>
                    <div className="flex items-center gap-3">
                        ...
                        <button
                            type="button"
                            onClick={addRow}
                            className="px-3 py-2 text-sm font-medium text-teal-700 dark:text-teal-300 bg-teal-50 dark:bg-teal-900/30 hover:bg-teal-100 dark:hover:bg-teal-900/50 rounded-lg inline-flex items-center gap-1.5 transition-colors"
                        >
                ...
                <div className="flex-1 p-8 max-w-4xl mx-auto w-full">
                    <div className="mb-6 p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 text-sm text-blue-800 dark:text-blue-200 flex gap-3 items-start">
                        ...
                        <div>
                            ...
                            <ul className="space-y-1 text-xs leading-relaxed">
                                ...
                                <li>• 특정 상품에만 다른 FAQ 를 보여주고 싶으면 해당 상품 편집 페이지에서 별도로 입력하세요 (우선 적용됨).</li>
                                <li>• 빈 상태로 저장하면 기본 FAQ 가 표시됩니다.</li>
                            </ul>
                        </div>
                    </div>
                        ...
                        <div className="space-y-3">
                            ...
                            {rows.length === 0 && (
                                <div className="text-center py-12 text-slate-400 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-dashed border-slate-300 dark:border-slate-700">
                                    등록된 FAQ 가 없습니다. 우상단 "질문 추가" 버튼으로 시작하세요.
                                </div>
                            )}
