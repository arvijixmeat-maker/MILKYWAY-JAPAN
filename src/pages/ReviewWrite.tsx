...
export const ReviewWrite: React.FC = () => {
    ...
    const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            alert('?ªÂÉè??Ç¢?É„Éó??Éº?â„Å´Â§±Êïó?ó„Åæ?ó„Åü??);
        }
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const me = meResponse?.user;
            if (!me) {
                alert('??Ç∞?§„É≥?åÂøÖË¶Å„Åß?ô„Ä?);
                return;
            }

            await api.reviews.create({
                user_id: me.id,
                author_name: me.name || me.email?.split('@')[0] || '?øÂêç',
                visit_date: selectedReservation.startDate.slice(0, 7) + ' Ë®™Âïè',
                rating: rating,
                product_name: selectedReservation.productName,
                title: `${selectedReservation.productName} ?¨„Éì?•„Éº`,
                content: content,
                images: images,
            ...
            alert('?¨„Éì?•„Éº??øùÂ≠ò„Å´Â§±Êïó?ó„Åæ?ó„Åü: ' + (error.message || 'Unknown error'));
        }
    };
        ...
        <div className="bg-background-light dark:bg-background-dark font-display text-[#0e1a18] dark:text-white min-h-screen">
            <div className="relative flex h-auto min-h-screen w-full flex-col bg-background-light dark:bg-background-dark group/design-root overflow-x-hidden">
                ...
                <div className="sticky top-0 z-50 flex items-center bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md p-4 justify-between border-b border-gray-100 dark:border-zinc-800">
                    ...
                    <h2 className="text-[#0e1a18] dark:text-white text-lg font-bold leading-tight tracking-[-0.015em] flex-1 text-center pr-10">?¨„Éì?•„Éº‰ΩúÊàê</h2>
                </div>

                <div className="flex-1 pb-32">
                    {/* Section: Tour Selection */}
                    <div className="px-4 pt-6">
                        <h3 className="text-[#0e1a18] dark:text-white text-xl font-bold leading-tight tracking-[-0.015em] mb-4">?ÄËøëÂèÇ?†„Åó?ü„ÉÑ?¢„Éº</h3>
                        <div className="flex flex-col gap-3">
                            {completedReservations && completedReservations.length > 0 ? (
                                ...
                                <div className="text-center py-8 text-gray-400">
                                    <p>ÂÆå‰∫Ü?ó„Åü?Ñ„Ç¢?º„ÅØ?Ç„Çä?æ„Åõ?ì„Ä?/p>
                                    <p className="text-xs pt-2">‰∫àÁ¥ÑÁ¢∫ÂÆö?ªÊóÖË°åÂÆå‰∫ÜÂæå?´‰Ωú?ê„Åß?ç„Åæ?ô„Ä?/p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="px-4 pt-8">
                        <h3 className="text-[#0e1a18] dark:text-white text-xl font-bold leading-tight tracking-[-0.015em] mb-2">‰ªäÂõû??óÖË°å„ÅØ?Ñ„Åã?å„Åß?ó„Åü?ãÔºü</h3>
                        <p className="text-gray-500 text-sm mb-6">?ü„ÅÆ?∞„Çí?∏Êäû?ó„Å¶?è„Å†?ï„ÅÑ??/p>
                    ...
                    <div className="px-4 pt-8">
                        <div className="flex justify-between items-end mb-4">
                            <h3 className="text-[#0e1a18] dark:text-white text-xl font-bold leading-tight tracking-[-0.015em]">?ôÁúü?¢„ÉÉ?ó„É≠?º„Éâ</h3>
                        ...
                        <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                            ...
                            <label className="flex flex-col items-center justify-center shrink-0 size-24 bg-white dark:bg-[#1a2e2a] border-2 border-dashed border-gray-200 dark:border-zinc-700 rounded-xl cursor-pointer hover:bg-gray-50 dark:hover:bg-zinc-800 transition-colors">
                                ...
                                <span className="text-[10px] text-gray-400 mt-1 font-bold">ËøΩÂä†</span>
                            </label>
                    ...
                    <div className="px-4 pt-8 pb-10">
                        <h3 className="text-[#0e1a18] dark:text-white text-xl font-bold leading-tight tracking-[-0.015em] mb-4">Ë©≥Á¥∞?¨„Éì?•„Éº</h3>
                        <textarea
                            value={content}
                            ...
                            placeholder="?¢„É≥?¥„É´?ß„ÅÆÂ§ßÂàá?™ÊÄù„ÅÑ?∫„Çí?±Êúâ?ó„Å¶?è„Å†?ï„ÅÑÔºàÂ§©Ê∞ó„ÄÅ„Ç¨?§„Éâ?ÅÂ?Ê≥äÊñΩË®?Å™?©Ëá™?±„Å™?îÊÑèË¶ãÔºâ"
                        ></textarea>
                    </div>
                </div>
                    ...
                    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 backdrop-blur-[2px]">
                        <div className="mx-6 w-full max-w-[320px] transform overflow-hidden rounded-2xl bg-white dark:bg-zinc-800 p-6 shadow-2xl transition-all animate-in fade-in zoom-in duration-200">
                            ...
                            <div className="flex pt-2">
                                ...
                                >
                                    <span className="truncate">Á¢∫Ë™ç</span>
                                </button>
                            </div>
                        </div>
                    </div>
