...
export const AdminQuoteManage: React.FC = () => {
    ...
    const [filterStatus, setFilterStatus] = useState('?ÑÏ≤¥ ?ÅÌÉú');
    // Pagination State
    ...
    const filteredRequests = useMemo(() => {
        return requests.filter(req => {
            ...
            const matchesStatus = filterStatus === '?ÑÏ≤¥ ?ÅÌÉú' ||
                (filterStatus === '?µÎ? ?ÄÍ∏? && req.status === 'new') ||
                (filterStatus === '?ÅÎã¥ Ï§? && req.status === 'processing') ||
                (filterStatus === '?µÎ? ?ÑÎ£å' && req.status === 'answered') ||
                (filterStatus === '?àÏïΩ ?ÑÌôò' && req.status === 'converted') ||
                (filterStatus === '?àÏïΩ ?îÏ≤≠' && req.status === 'reservation_requested');
    ...
    const bankAccount = {
        ...
        accountHolder: 'Î™ΩÍ≥®Î¶¨ÏïÑ ?Ä?òÏàò'
    };
    ...
    const handleConvertToReservation = async (data: { startDate: string; endDate: string; totalAmount: number; deposit: number }) => {
        ...
        try {
            ...
            if (confirm('?àÏïΩ???±Í≥µ?ÅÏúºÎ°??ùÏÑ±?òÏóà?µÎãà??\n[?àÏïΩ Í¥ÄÎ¶? ?òÏù¥ÏßÄÎ°??¥Îèô?òÏó¨ ?ïÏù∏?òÏãúÍ≤†Ïäµ?àÍπå?')) {
                navigate('/admin/reservations');
            }

        } catch (error: any) {
            console.error("Failed to convert reservation:", error);
            alert(`?àÏïΩ Î≥Ä??Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§: ${error.message || error}`);
        }
    };
    ...
    const handleSendEstimate = async (url: string, note: string, priceDetail: any, confirmedStartDate: string, confirmedEndDate: string) => {
        ...
        try {
            ...
            alert(`Í≤¨Ï†Å??Î∞úÏÜ° ?ÑÎ£å!\nURL: ${url}\n?ïÏ†ï Í∏àÏï°: ${priceDetail.totalAmount ? priceDetail.totalAmount.toLocaleString() + '?? : 'ÎØ∏ÏûÖ??}\n(Í≥†Í∞ù?êÍ≤å Gmail ?åÎ¶º??Î∞úÏÜ°?òÏóà?µÎãà??`);
            ...
            alert("Í≤¨Ï†Å??Î∞úÏÜ° Ï≤òÎ¶¨ Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.");
        }
    };
    ...
    const handleUpdateQuote = async (id: string, updates: Partial<QuoteRequest>) => {
        try {
            ...
            alert("?ïÎ≥¥ ?òÏ†ï Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.");
        }
    };
    ...
    const StatusDropdown = ({ status, onChange }: { status: string, onChange: (s: QuoteRequest['status']) => void }) => {
        ...
        const labels: Record<string, string> = {
            pending_payment: '?ÖÍ∏à ?ÄÍ∏?,
            paid: 'Í≤∞Ï†ú ?ÑÎ£å',
            confirmed: '?àÏïΩ ?ïÏ†ï',
            cancelled: 'Ï∑®ÏÜå??,
            converted: '?àÏïΩ ?ÑÌôò',
            reservation_requested: '?àÏïΩ ?îÏ≤≠'
        };
        ...
        <div className="flex min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 font-sans">
            ...
            <main className="ml-64 flex-1 flex flex-col min-h-screen">
                ...
                <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-40 px-8 flex items-center justify-between">
                    <h1 className="text-xl font-bold text-slate-800 dark:text-white">Í≤¨Ï†Å Í¥ÄÎ¶?Î¶¨Ïä§??/h1>
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-3 pl-4 border-l border-slate-200 dark:border-slate-800">
                            <span className="text-sm font-medium text-slate-600 dark:text-slate-300">Í¥ÄÎ¶¨Ïûê?? ?òÏòÅ?©Îãà??/span>
                ...
                <div className="p-8 space-y-6 bg-slate-50 dark:bg-slate-900">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between">
                            <div>
                                <p className="text-sm text-slate-500 dark:text-slate-400 font-medium mb-1">?ÑÏ≤¥ ?îÏ≤≠</p>
                                <h3 className="text-3xl font-bold text-slate-800 dark:text-white">{stats.total}Í±?/h3>
                            </div>
                        ...
                        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between">
                            <div>
                                <p className="text-sm text-slate-500 dark:text-slate-400 font-medium mb-1">?µÎ? ?ÄÍ∏?/p>
                                <h3 className="text-3xl font-bold text-slate-800 dark:text-white">{stats.new}Í±?/h3>
                            </div>
                        ...
                        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between">
                            <div>
                                <p className="text-sm text-slate-500 dark:text-slate-400 font-medium mb-1">?ÑÎ£å??Í≤¨Ï†Å</p>
                                <h3 className="text-3xl font-bold text-slate-800 dark:text-white">{stats.completed}Í±?/h3>
                            </div>
                            <div className="w-12 h-12 rounded-xl bg-teal-50 dark:bg-teal-900/20 text-teal-500 flex items-center justify-center">
                    ...
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                            <div className="md:col-span-4">
                                <label className="block text-xs font-bold text-slate-400 mb-2">?†Ïßú Î≤îÏúÑ</label>
                                <div className="relative">
                            ...
                            <div className="md:col-span-4">
                                <label className="block text-xs font-bold text-slate-400 mb-2">Í≥†Í∞ùÎ™?Í≤Ä??/label>
                                <div className="relative">
                                    <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-lg">search</span>
                                    <input
                                        type="text"
                                        placeholder="Í≥†Í∞ù ?¥Î¶Ñ ?êÎäî ?¨ÌñâÏßÄ Í≤Ä??
                                        value={searchTerm}
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-xs font-bold text-slate-400 mb-2">?ÅÌÉú</label>
                                <select
                                    value={filterStatus}
                                ...
                                >
                                    <option>?ÑÏ≤¥ ?ÅÌÉú</option>
                                    <option>?µÎ? ?ÄÍ∏?/option>
                                    <option>?ÅÎã¥ Ï§?/option>
                                    <option>?µÎ? ?ÑÎ£å</option>
                                    <option>?àÏïΩ ?îÏ≤≠</option>
                                    <option>?àÏïΩ ?ÑÌôò</option>
                                </select>
                            </div>
                            <div className="md:col-span-2">
                                <button onClick={() => { setSearchTerm(''); setFilterStatus('?ÑÏ≤¥ ?ÅÌÉú'); }} className="w-full py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold rounded-lg transition-colors flex items-center justify-center gap-2">
                        ...
                        <div className="overflow-x-auto min-h-[400px]">
                            <table className="w-full text-left border-collapse">
                                <thead className="bg-slate-50 dark:bg-slate-700/50">
                                    <tr>
                                        <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider">?îÏ≤≠??/th>
                                        <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Í≥†Í∞ùÎ™?/th>
                                        <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider">?¨Îßù ?¨ÌñâÏßÄ</th>
                                        <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider">?∏Ïõê</th>
                                        <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider">?¨Îßù ?ºÏ†ï</th>
                                        <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider text-center">?ÅÌÉú</th>
                                        <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider text-center">Í¥ÄÎ¶?/th>
                                    </tr>
                                </thead>
