...
// .wrangler/tmp/pages-SqrIHM/functionsWorker-0.25352502870356186.mjs
  ...
  route(path, app21) {
    ...
    app21.routes.map((r) => {
      ...
      if (app21.errorHandler === errorHandler) {
        ...
        handler = /* @__PURE__ */ __name2(async (c, next) => (await compose([], app21.errorHandler)(c, () => r.handler(c, next))).res, "handler");
...
var handle = /* @__PURE__ */ __name2((app21) => (eventContext) => {
  return app21.fetch(
...
var app = new Hono2();
var safeParse = /* @__PURE__ */ __name2((val, fallback = []) => {
...
var toJson = /* @__PURE__ */ __name2((val) => {
  if (typeof val === "string") return val;
  return JSON.stringify(val || []);
}, "toJson");
var g = /* @__PURE__ */ __name2((data, snakeKey, camelKey, defaultVal = "") => {
...
app.get("/", async (c) => {
  ...
  try {
    ...
    const parsed = (result.results || []).map((p) => ({
      ...p,
...
app.post("/reorder", async (c) => {
  try {
    ...
    const statements = items.map(
      (item) => db.prepare("UPDATE products SET sort_order = ? WHERE id = ?").bind(item.sortOrder, item.id)
    );
...
var TimeSpan = class _TimeSpan {
  static {
    __name(this, "_TimeSpan");
  }
  static {
    __name2(this, "TimeSpan");
  }
...
var DrizzleSQLiteAdapter = class {
  ...
  static {
    __name2(this, "DrizzleSQLiteAdapter");
  }
  db;
  sessionTable;
  userTable;
...
var users = sqliteTable("users", {
  ...
  // For admin email/password login
});
...
function initializeLucia(D1) {
  ...
  return new Lucia(adapter, {
    ...
    getUserAttributes: /* @__PURE__ */ __name2((attributes) => {
      return {
...
app2.get("/", async (c) => {
  ...
  const parsed = result.map((r) => ({
    ...
    dailyAccommodations: r.dailyAccommodations ? JSON.parse(r.dailyAccommodations) : void 0,
    history: r.history ? JSON.parse(r.history) : void 0
  }));
  return c.json(parsed);
});
app2.get("/:id", async (c) => {
  ...
  const parsed = {
    ...
    dailyAccommodations: result.dailyAccommodations ? JSON.parse(result.dailyAccommodations) : void 0,
    history: result.history ? JSON.parse(result.history) : void 0
  };
  return c.json(parsed);
});
app2.put("/:id", async (c) => {
  ...
  const parsed = {
    ...
    dailyAccommodations: updated?.dailyAccommodations ? JSON.parse(updated.dailyAccommodations) : void 0,
    history: updated?.history ? JSON.parse(updated.history) : void 0
  };
  return c.json(parsed);
});
app2.post("/", async (c) => {
...
app3.get("/login/google", async (c) => {
  const google = new Google(c.env.GOOGLE_CLIENT_ID, c.env.GOOGLE_CLIENT_SECRET, "http://localhost:5173/api/auth/login/google/callback");
...
app3.get("/login/google/callback", async (c) => {
...
var notifications_default = app5;
var app6 = new Hono2();
app6.get("/", async (c) => {
  const db = c.env.DB;
  try {
...
app8.put("/bulk", async (c) => {
  const { faqs } = await c.req.json();
  const db = c.env.DB;
  try {
    const stmts = [db.prepare("DELETE FROM faqs")];
...
var accommodations_default = app11;
var app12 = new Hono2();
app12.get("/", async (c) => {
  const db = c.env.DB;
...
var products = sqliteTable("products", {
  ...
  mainImages: text("main_images").notNull(),
  // JSON.stringify(string[])
  galleryImages: text("gallery_images").notNull(),
  // JSON.stringify(string[])
  detailImages: text("detail_images").notNull(),
  // JSON.stringify(string[])
  itineraryImages: text("itinerary_images").notNull(),
  // JSON.stringify(string[])
...
app14.put("/:id", async (c) => {
  ...
  await db.update(quotes).set({
    ...
    // Ensure camelCase vs snake_case matches schema
...
var event_banners_default = app18;
var app19 = new Hono2();
app19.get("/", async (c) => {
  try {
...
var migrate_db_default = app19;
var app20 = new Hono2().basePath("/api");
app20.get("/", (c) => {
  return c.json({ message: "Milkyway Japan API" });
});
app20.route("/products", products_default);
app20.route("/reservations", reservations_default);
app20.route("/auth", auth_default);
app20.route("/notifications/email", email_default);
app20.route("/notifications", notifications_default);
app20.route("/banners", banners_default);
app20.route("/settings", settings_default);
app20.route("/faqs", faqs_default);
app20.route("/reviews", reviews_default);
app20.route("/magazines", magazines_default);
app20.route("/accommodations", accommodations_default);
app20.route("/categories", categories_default);
app20.route("/guides", guides_default);
app20.route("/quotes", quotes_default);
app20.route("/upload", upload_default);
app20.route("/images", path_default);
app20.route("/quick-links", quick_links_default);
app20.route("/event-banners", event_banners_default);
app20.route("/migrate-db", migrate_db_default);
var onRequest = handle(app20);
var SEO_CONSTANTS = {
  TITLE: "\u30E2\u30F3\u30B4\u30EB\u65C5\u884C\u30FB\u30E2\u30F3\u30B4\u30EB\u30C4\u30A2\u30FC\u5C02\u9580 | \u73FE\u5730\u65C5\u884C\u793E Milkyway Japan",
  DESCRIPTION: "\u30E2\u30F3\u30B4\u30EB\u65C5\u884C\u3001\u30E2\u30F3\u30B4\u30EB\u30C4\u30A2\u30FC\u306A\u3089\u30E2\u30F3\u30B4\u30EB\u5C02\u9580\u306E\u73FE\u5730\u65C5\u884C\u793EMilkyway Japan\u3078\u3002\u590F\u306E\u30E2\u30F3\u30B4\u30EB\u3001\u30E2\u30F3\u30B4\u30EB\u4E57\u99AC\u65C5\u884C\u3001\u30B4\u30D3\u7802\u6F20\u30C4\u30A2\u30FC\u306A\u3069\u3001\u591A\u5F69\u3067\u9B45\u529B\u7684\u306A\u30E2\u30F3\u30B4\u30EB\u65C5\u884C\u5546\u54C1\u3092\u3054\u63D0\u6848\u3057\u307E\u3059\u3002",
  KEYWORDS: [
    "\u30E2\u30F3\u30B4\u30EB\u65C5\u884C",
    "\u30E2\u30F3\u30B4\u30EB\u30C4\u30A2\u30FC",
    "\u30E2\u30F3\u30B4\u30EB\u73FE\u5730\u65C5\u884C\u793E",
...
var onRequestGet = /* @__PURE__ */ __name2(async (context) => {
  try {
    ...
    if (magazinesResult && magazinesResult.results) {
      magazinesResult.results.forEach((mag) => {
...
var getAbsoluteImageUrl = /* @__PURE__ */ __name2((url) => {
...
// .wrangler/tmp/bundle-ZNJSpt/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ZNJSpt/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.25352502870356186.js.map
